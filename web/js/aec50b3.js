var game = {}; 
	...
	$(document).ready(function(){
		function initGame(){ 
			...
			game.playerNames = {1:player1,2:player2}; 
			game.isClick = 0;
			...
			game.getPlayerName = function(p){    
				if (isNaN(p)){ 
					return (game.playerNames[1] == p)? 1:2; 
				}; 
				return (1 == p)? game.playerNames[1]:game.playerNames[2];
			};
						   ...
						   success: function(msg){
								...
								if (jobj.errorMessage){ 
									switch(jobj.errorMessage[0]){
									   ...
									   type: "POST",
									   url: "/recordWinner",
									   data: {player1:player1,player2:player2,winner:'none'},
									   success: function(msg){
											}});
										...
										default: 
											setTimeout(function(){
												winner = game.getPlayerName(jobj.errorMessage); 
												$.ajax({
											   type: "POST",
											   url: "/recordWinner",
											   data: {player1:player1,player2:player2,winner:winner},
											   success: function(msg){
													}});
